// ─────────────────────────────────────────────────────────────────────────
// Cloudflare Pages Function — Kiosco de Acreditación BAIRES MODELFEST 2026
//
// Este archivo va en  functions/api/jf.js  del proyecto de Cloudflare Pages
// del kiosco. Es el ÚNICO lugar donde vive la API Key de JotForm — nunca
// viaja al kiosco/tablet, así la página puede ser pública sin que nadie
// pueda copiarla desde el código fuente.
//
// El kiosco le pega a "/api/jf" (ruta relativa, mismo dominio) con:
//   { op: "get" | "post" | "delete", path: "...", params: {...}, formFields: [[k,v], ...] }
// y este proxy arma la llamada real a api.jotform.com agregando la key.
//
// Mismo mecanismo que ya usa la app de Juzgamiento — si ese proyecto
// también corre en Cloudflare Pages, este archivo es intercambiable.
//
// CONFIGURACIÓN NECESARIA EN CLOUDFLARE:
//   Pages → (proyecto del kiosco) → Settings → Environment variables
//   → Add variable → nombre: JOTFORM_API_KEY, valor: (la misma key que
//   ya usaba el HTML viejo) → Type: Secret → guardar → re-deploy.
// ─────────────────────────────────────────────────────────────────────────
